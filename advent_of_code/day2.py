...
from advent_of_code import constants, util
...
def main():
    ...
    print(count_safe_reports(constants.INPUT_DAY2_PATH))
    util.print_output_string(2, 2)
    print(count_safe_reports(constants.INPUT_DAY2_PATH, True))
...
def is_report_safe(report: List[str], allow_bad_level=False) -> bool:
    ...
    for i in range(1, length - 1):
        ...
        if not (prev_lvl < current_lvl < next_lvl):
            if not (prev_lvl > current_lvl > next_lvl):
                ...
                return (
                    is_report_safe_without_item_at_index(report, i - 1)
                    or is_report_safe_without_item_at_index(report, i)
                    or is_report_safe_without_item_at_index(report, i + 1)
                )
        ...
        if not difference_is_between_one_and_three(prev_lvl, current_lvl):
            if not allow_bad_level:
                return False
            return is_report_safe_without_item_at_index(report, i - 1) or is_report_safe_without_item_at_index(
                report, i
            )
    ...
    if difference_is_between_one_and_three(int(report[-1]), int(report[-2])):
        ...
        return is_report_safe_without_item_at_index(report, -1) or is_report_safe_without_item_at_index(report, -2)
    return False


def is_report_safe_without_item_at_index(report: List[str], index: int) -> bool:
